...
public class BuyMapCoins : MonoBehaviour
{
    ...
    public Text coinsCount, slowMotionCountText;

    public void BuyNewMap(int needCoins)
    {
        int coins = PlayerPrefs.GetInt("Coins");
        int slowMotionCount = PlayerPrefs.GetInt("SlowMotionScore");
        if (coins < needCoins)
        {
        ...
        else
        {
            ...
            switch (needCoins)
            {
                ...
                case 100:
                    slowMotionCount++;
                    PlayerPrefs.SetInt("SlowMotionScore", slowMotionCount);
                    int slowMotionScore = PlayerPrefs.GetInt("SlowMotionScore");
                    PlayerPrefs.Save();
                    slowMotionCountText.text = slowMotionScore.ToString();
                    break;
            }
